...
export function Work() {
  ...
  useGSAP(
    () => {
      ...
      if (reducedMotion) {
        gsap.set(imgRefs.current.filter(Boolean), { scale: 1, xPercent: 0, yPercent: 0 });
      ...
      if (horizontal) {
        ...
        gsap.set(imgRefs.current.filter(Boolean), { scale: 1.15 });
        ...
        tl.to(trackRef.current, { x: () => -getDistance(), ease: "none" }, 0);
        imgRefs.current.forEach((img) => {
          if (img) tl.to(img, { xPercent: -10, ease: "none" }, 0);
        });

        gsap.set([...imgWrapRefs.current, ...textColRefs.current].filter(Boolean), { opacity: 1, y: 0 });
        return;
      }
    ...
    { scope: rootRef, dependencies: [reducedMotion, horizontal] }
  );
  ...
  const renderMeta = (project, i) => (
    <>
      ...
      <p className="max-w-[46ch] text-[16px] leading-[1.6] md:text-[17px]" style={{ color: MUTED }}>
        {project.description}
      </p>
  ...
  return (
    <section id="work" ref={rootRef} className="w-full bg-[#0F1115]" style={{ minHeight: "100vh" }}>
      <div className="mx-auto max-w-[1440px] px-6 pb-24 pt-24 md:px-12 md:pb-32 md:pt-32 lg:pt-40">
        {/* header */}
        <div className={horizontal ? "mb-16 lg:mb-24" : "mb-24 md:mb-32 lg:mb-48"}>
          <div style={{ borderTop: `1px solid ${HAIRLINE}` }} />
          <p className="mt-6 text-[11px] uppercase" style={{ color: MUTED, letterSpacing: "0.2em" }}>
            02 / WORK
          </p>
            ...
            {projects.map((project, i) => {
              ...
              return (
                <article
                  ...
                  onMouseEnter={() => handleEnter(i)}
                  onMouseLeave={() => handleLeave(i)}
                  className="grid h-full w-screen shrink-0 grid-cols-1 items-center gap-10 px-6 md:px-12 lg:grid-cols-12 lg:gap-x-20 lg:px-20"
                >
                  <a
                    href={project.href}
                    ref={(el) => {
                    ...
                    className="relative block aspect-[4/3] overflow-hidden rounded-[14px] lg:col-span-6"
                    aria-label={`View ${project.title} project`}
                  >
                    <img
                      ref={(el) => {
                  ...
                  <div
                    ...
                    className="flex flex-col gap-6 lg:col-span-5 lg:col-start-8"
                  >
                    <span className="font-mono text-[11px] uppercase" style={{ color: MUTED, letterSpacing: "0.2em" }}>
